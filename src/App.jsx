...
import { PublicFaculty, PublicEvents, PublicBus, PublicPrayer, PublicRoutines, PublicCoverPage, PublicAnnouncements, PublicCalendar, PublicCGPA } from "./screens/public/Explore.jsx";
...
import Reports from "./screens/student/Reports.jsx";
...
export default function App() {
    ...
    return (
      <div className="flex min-h-screen flex-col items-center justify-center gap-4 bg-bg px-6 text-center">
        <div className="max-w-sm">
          <h1 className="text-2xl font-bold text-ink">Couldn't load your profile</h1>
          <p className="mt-1 text-base text-ink-3">Something went wrong fetching your account. Check your connection and try again.</p>
        </div>
        <div className="flex gap-3">
          <button onClick={retryProfile} className="inline-flex h-10 items-center gap-2 rounded-md bg-brand px-4 text-base font-bold text-white hover:bg-brand-700">Retry</button>
          <button onClick={() => logout()} className="inline-flex h-10 items-center gap-2 rounded-md border border-brd bg-surface px-4 text-base font-semibold text-ink-2 hover:bg-surface-2">Sign out</button>
        </div>
      </div>
    );
  }
  ...
  if (path === "/explore/announcements") return <PublicAnnouncements />;
  if (path === "/explore/calendar") return <PublicCalendar />;
  if (path === "/explore/bus") return <PublicBus />;
  if (path === "/explore/prayer") return <PublicPrayer />;
  if (path === "/explore/routines") return <PublicRoutines />;
  if (path === "/explore/cover-page") return <PublicCoverPage />;
  if (path === "/explore/cgpa") return <PublicCGPA />;
  if (path === "/explore/campus") return <Redirect to="/explore/bus" />;
...
function AuthedRoutes({ path }) {
  ...
  // Two routes, one page — the path picks the tab (see screens/student/Reports.jsx).
  if (path === "/reports") return <RequireRole role="Student"><Reports tab="mine" /></RequireRole>;
  if (path === "/campus-issues") return <RequireRole role="Student"><Reports tab="board" /></RequireRole>;
